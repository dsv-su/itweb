<div class="min-w-0">
    ...
    <div class="flex min-w-0 items-stretch gap-3">
        <div class="relative min-w-0 flex-1" x-data="{open:false}" x-on:click.away="open=false" @keydown.escape.stop.prevent="open=false; $refs.trigger.focus()">
            <button id="project-select" @error('project') aria-invalid="true" @enderror aria-describedby="@if($showProjectWarning && !$Project->exists) project-missing-warning @endif @error('project') project-error @enderror" x-ref="trigger" aria-labelledby="project-label project-value" :aria-expanded="open.toString()" aria-controls="project-options" type="button" class="font-mono bg-gray-50 dark:bg-gray-700 border border-gray-500 dark:border-gray-400 text-gray-900 dark:text-gray-200 p-2.5 rounded-lg shadow-inner w-full flex justify-between items-center gap-2 text-sm focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-blue-700 dark:focus-visible:outline-blue-300" x-on:click="open=!open; if(open) $nextTick(() => $refs.search.focus())">
                <span id="project-value" class="min-w-0 break-words text-left">
                    {{ $Project->project ?? __('Select Project') }}
                    @if(filled($Project->project) && filled($Project->description))
                        <span class="block font-sans text-sm leading-5 text-gray-700 dark:text-gray-300">{{ $Project->description }}</span>
                    @endif
                </span>
        ...
        @if($showUpdateButton)
            <button type="submit" form="{{ $updateFormId }}" name="decision" value="update"
                    wire:loading.attr="disabled"
                    class="inline-flex shrink-0 items-center justify-center rounded-lg border border-red-600 px-4 py-2.5 text-sm font-semibold text-red-600 hover:bg-red-600 hover:text-white focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-red-600 disabled:cursor-wait disabled:opacity-50 dark:border-red-400 dark:text-red-400 dark:hover:bg-red-600 dark:hover:text-white">
                {{ __('Update') }}
            </button>
        @endif
    </div>
