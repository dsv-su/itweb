<div class="mt-5 border rounded-xl shadow-sm p-6 dark:bg-slate-800 dark:border-gray-700">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
            <p class="text-sm font-medium text-gray-900 dark:text-white">Sent notification recipients</p>
            <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Selected SUKAT users receive an email when a proposal is registered as sent.</p>
    ...
    @if (session()->has('sent_notification_recipients_saved'))
        <div class="mt-3 text-sm text-green-700 dark:text-green-400">
            {{ session('sent_notification_recipients_saved') }}
        </div>
    @endif
    ...
    @if(count($recipients) > 0)
        <div class="mt-4">
            @foreach($recipients as $key => $recipient)
                ...
                <div wire:key="sent-recipient-row-{{ $key }}" class="flex flex-col md:flex-row gap-4 mb-4 items-center transition {{ $isStored ? '' : 'opacity-70' }}">
                    <div
    ...
    <div class="flex justify-start">
        <div class="relative w-full"
            ...
            <input
                ...
                aria-controls="sent-recipient-search-results">

            @if(filled($searchRecipient))
                <div id="sent-recipient-search-results" role="listbox"
                    ...
                    <ul class="space-y-2">
                        @foreach($sukatUsers as $i => $sukatUser)
                            <li wire:key="sent-recipient-{{ $sukatUser->uid ?? $sukatUser->name }}">
                                <button
                                    type="button"
                                    role="option"
